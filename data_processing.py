...
class DB:
    ...
    def insert(self,table_object):
        self.tables[table_object.table_name] = table_object
...
class Table:
    ...
    def filter(self,condition):
        ...
        for i in self.table:
            if condition(i):
                temp.append(i)
        return Table(self.table_name + "filtered",temp)
    
    def aggregate(self, aggregation_function, aggregation_key):
        values = []
        for i in self.table:
            val = i[aggregation_key]
            try:
                val = float(val)
            except ValueError:
                pass
    ...
    def join(self,other_table,join_column_name):
        joined_data = []
        for row_self in self.table:
            for row_other in other_table.table:
                if join_column_name in row_self and join_column_name in row_other:
                    if row_self[join_column_name] == row_other[join_column_name]:
                        new_row = {**row_self,**row_other}
                        joined_data.append(new_row)
                        
        new_table_name = f"{self.table_name}_joined_{other_table.table_name}"
